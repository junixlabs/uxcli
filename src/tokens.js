...
// Aliases: `--fg-subtle: var(--ink-500)`. Same read-only scan; returns { alias: Set(targets) }: a theme block that re-points an alias makes it ambiguous, which the caller reports.
export function tokenAliases(root) {
  const out = {}; if (!root || !fs.existsSync(root)) return out;
  const walk = d => { for (const f of fs.readdirSync(d, { withFileTypes: true })) { if (f.name === 'node_modules' || f.name.startsWith('.')) continue; const p = path.join(d, f.name); if (f.isDirectory()) walk(p); else if (EXT.has(path.extname(f.name))) for (const m of fs.readFileSync(p, 'utf8').matchAll(/(--[a-zA-Z0-9_-]+)\s*:\s*var\((--[a-zA-Z0-9_-]+)\)/g)) (out[m[1]] ||= new Set()).add(m[2]); } };
