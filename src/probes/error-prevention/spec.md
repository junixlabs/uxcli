...
v9.1 2026-09-07: the planted-error pass moved to a module shared with 3.3.1 (`src/plant.js`), one planting per journey. Tightened: the browser's constraint validation counts as the *checked* message only where the browser enforces it, that is the form has no `novalidate` and no `formnovalidate` submitter; a `novalidate` form still populates `validationMessage` without showing or blocking anything, so it was a latent false pass of the *checked* branch (found by the 3.3.1 must-fail twin, which is exactly such a form).
